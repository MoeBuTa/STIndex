"""
QA evaluation pipeline using RRF-4 retrieval and MIRAGE questions.

Evaluates:
1. Retrieval quality (Recall@k, Precision@k)
2. QA performance (Exact Match, Token F1)
3. Answer coverage in retrieved context

Usage:
    python -m stindex.eval.qa_evaluation \\
...
from stindex.eval.qa_metrics import QAMetrics
...
Based on the context provided, answer with the correct option letter (A, B, C, or D) only. Do not provide any explanation."""
    ...
    def evaluate_question(self, question: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get predicted answer (baseline: match options to context)
...
def run_qa_evaluation(
    ...
    Args:
        questions_file: Path to filtered MIRAGE questions
        corpus_path: Path to corpus JSONL
        indices_dir: Directory with FAISS indices
